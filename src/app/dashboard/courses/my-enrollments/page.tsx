      ...
      {!loading && enrollments.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4"
            ...
            <svg className="w-7 h-7 opacity-50" viewBox="0 0 24 24" fill="none" stroke="#00F5FF" strokeWidth="1.5">
          ...
          <Link href="/dashboard/courses"
            ...
            style={{ color: '#00F5FF', textDecoration: 'none', border: '1px solid rgba(0,245,255,0.25)', borderRadius: 8, padding: '7px 16px', display: 'inline-block' }}>
              ...
              <div key={e.id}
                ...
                <div style={{ display: 'flex', gap: 14, padding: 16 }}>
                  ...
                  {course.coverUrl ? (
                    ...
                    <div style={{ width: 76, height: 76, borderRadius: 10, flexShrink: 0, background: 'rgba(0,245,255,0.06)', border: '1px solid rgba(0,245,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg style={{ width: 28, height: 28, opacity: 0.3 }} viewBox="0 0 24 24" fill="none" stroke="#00F5FF" strokeWidth="1.5">
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '10px 16px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                  {e.status === 'APPROVED' && (
                    <Link href={`/dashboard/courses/${course.id}`}
                      style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: '#000', textDecoration: 'none', background: 'linear-gradient(135deg, #00F5FF 0%, #00FF88 100%)', borderRadius: 8, padding: '7px 14px', whiteSpace: 'nowrap' }}>
                      ▶ Ver videos
                    </Link>
                  )}
                  {e.status === 'REJECTED' && (
