...
function ChatsTab({ bot }: { bot: Bot }) {
  ...
  return (
    <div className="glass-panel rounded-2xl border border-white/[0.07] overflow-hidden" style={{ minHeight: 480 }}>
      <div className="flex" style={{ minHeight: 480 }}>

        {/* ── Panel izquierdo: lista de contactos ── */}
        {/* móvil: visible solo si no hay chat seleccionado | sm+: siempre visible, ancho fijo */}
        <div
          className={`flex flex-col border-r border-white/[0.07] ${showList ? 'flex w-full' : 'hidden'} sm:flex sm:w-56 sm:shrink-0`}
          style={{ background: 'rgba(255,255,255,0.01)' }}
        >
          <div className="p-3 border-b border-white/5 flex items-center justify-between shrink-0">
            <span className="text-xs font-bold text-dark-300 uppercase tracking-wider">Contactos</span>
            <button onClick={loadConvs} className="text-dark-500 hover:text-white transition-colors p-1" title="Refrescar">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {!loading && convs.length === 0 && (
              <div className="p-4 text-center text-xs text-dark-500">Sin conversaciones aún</div>
            )}
            {!loading && convs.map(c => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-3 border-b border-white/[0.04] transition-colors ${isSelected ? 'bg-neon-green/[0.08]' : 'hover:bg-white/[0.04]'}`}
                >
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <span className="text-xs font-semibold text-white truncate flex-1">
                      {c.userName || c.userPhone}
                    </span>
                    {c.sold && <span className="text-[9px] font-bold text-neon-green shrink-0">VENTA</span>}
                    {c.botDisabled && <span className="text-[9px] font-bold text-red-400 shrink-0">OFF</span>}
                  </div>
                  <p className="text-[10px] text-dark-500 truncate">
                    {lastMsg
                      ? (lastMsg.role === 'assistant' ? '🤖 ' : '👤 ') + parseAssistantContent(lastMsg.content).slice(0, 35)
                      : 'Sin mensajes'}
                  </p>
        ...
        {/* ── Panel derecho: chat ── */}
        {/* móvil: visible solo si hay chat seleccionado | sm+: siempre visible */}
          ...
          {!selectedPhone ? (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center px-4">
                ...
                <p className="text-xs text-dark-500">Selecciona un contacto</p>
              </div>
            </div>
          ) : (
            <>
              {/* Cabecera del chat */}
              <div className="px-3 sm:px-4 py-2.5 border-b border-white/5 flex items-center gap-2 shrink-0" style={{ background: 'rgba(255,255,255,0.02)' }}>
                {/* Botón volver — solo en móvil */}
                <button
                  onClick={() => setSelectedPhone(null)}
                  className="sm:hidden p-1.5 rounded-lg text-dark-400 hover:text-white hover:bg-white/5 transition-colors shrink-0"
                  title="Volver a contactos"
                >
                  <ArrowLeft className="w-4 h-4" />
                </button>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white truncate">{selectedConv?.userName || selectedPhone}</p>
                  <p className="text-[10px] text-dark-500 truncate">{selectedPhone}</p>
                </div>
                {/* Toggle bot */}
                <button
                  onClick={() => selectedConv && toggleBot(selectedPhone, selectedConv.botDisabled)}
                  disabled={togglingBot}
                  title={selectedConv?.botDisabled ? 'Activar bot' : 'Desactivar bot'}
                  className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold border transition-all disabled:opacity-50 shrink-0 ${
                    selectedConv?.botDisabled
                      ? 'bg-red-500/10 text-red-400 border-red-500/20 hover:bg-neon-green/10 hover:text-neon-green hover:border-neon-green/20'
                      : 'bg-neon-green/10 text-neon-green border-neon-green/20 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20'
                  }`}
                >
                  {togglingBot
                  ...
                  <span className="hidden xs:inline">{selectedConv?.botDisabled ? 'OFF' : 'ON'}</span>
                </button>
                {/* Borrar chat */}
                <button
                  ...
                  className="p-1.5 rounded-lg text-dark-500 hover:text-red-400 hover:bg-red-500/10 transition-colors disabled:opacity-50 shrink-0"
                >
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2" style={{ minHeight: 0, maxHeight: 340 }}>
                ...
                {!loadingMsgs && messages.length === 0 && (
                  <div className="flex items-center justify-center py-10 text-xs text-dark-500">Sin mensajes</div>
                )}
                {!loadingMsgs && messages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isBot ? 'justify-start' : 'justify-end'}`}>
                      <div
                        className="max-w-[85%] sm:max-w-[75%] rounded-xl px-3 py-2 text-xs leading-relaxed"
                        style={{
                          background: isBot ? 'rgba(0,245,255,0.08)' : 'rgba(0,255,136,0.08)',
                          border: isBot ? '1px solid rgba(0,245,255,0.15)' : '1px solid rgba(0,255,136,0.15)',
                          color: isBot ? 'rgba(0,245,255,0.9)' : 'rgba(0,255,136,0.9)',
                        }}
                      >
                        <div className="whitespace-pre-wrap break-words">{text}</div>
                        <div className="mt-1 text-[9px] opacity-40">
                          {isBot ? '🤖 Bot' : msg.type !== 'text' ? `📎 ${msg.type}` : '👤 Usuario'}
              ...
              {/* Input de respuesta */}
              {bot.type !== 'YCLOUD' ? (
                <div className="px-3 py-2.5 border-t border-white/5 text-center text-[10px] text-dark-500 shrink-0" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  Envío manual disponible solo para bots YCloud
                </div>
              ) : (
                <div className="px-3 py-2 border-t border-white/5 flex flex-col gap-1.5 shrink-0" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  {sendError && (
                  ...
                  <div className="flex gap-2 items-end">
                    <textarea
                      ...
                      rows={1}
                      className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-white placeholder-white/25 focus:outline-none focus:border-neon-green/40 transition-colors resize-none"
                      style={{ maxHeight: 80 }}
                    />
                    <button
                      onClick={sendReply}
                      disabled={sending || !replyText.trim()}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-neon-green/10 text-neon-green border border-neon-green/20 hover:bg-neon-green/20 transition-colors disabled:opacity-40 shrink-0"
                    >
