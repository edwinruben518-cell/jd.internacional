...
export default function CartPage() {
    ...
    return (
      <div className="px-4 sm:px-6 pt-8 pb-12 max-w-lg mx-auto">
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: 32, textAlign: 'center' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: '14px 18px', marginBottom: 20, textAlign: 'left' }}>
            ...
            {success.txHash && (
              <div style={{ marginTop: 10 }}>
                ...
                <a href={`https://bscscan.com/tx/${success.txHash}`} target="_blank" rel="noreferrer"
                  style={{ fontSize: 11, color: '#00F5FF', wordBreak: 'break-all', textDecoration: 'none' }}>
                  {success.txHash} ↗
                </a>
              </div>
            )}
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <Link href="/dashboard/store/my-orders" style={{ flex: 1, padding: '11px 16px', borderRadius: 10, fontWeight: 700, fontSize: 13, textAlign: 'center', textDecoration: 'none', background: 'linear-gradient(135deg, #00F5FF, #00FF88)', color: '#000', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
              Ver mis pedidos
            </Link>
  ...
  if (cart.length === 0) {
    return (
      <div className="px-4 sm:px-6 pt-8 pb-12 max-w-lg mx-auto text-center">
        ...
        <Link href="/dashboard/store" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '11px 22px', borderRadius: 10, fontWeight: 700, fontSize: 13, textDecoration: 'none', background: 'linear-gradient(135deg, #00F5FF, #00FF88)', color: '#000' }}>
          Ir a la tienda
        </Link>
      </div>
    )
  ...
  return (
    <div className="px-4 sm:px-6 pt-6 pb-12 max-w-2xl mx-auto">
      ...
      <div className="h-px w-16 mb-6 rounded-full" style={{ background: 'linear-gradient(90deg, transparent, #00F5FF, #FF2DF7, transparent)' }} />

      {/* ── SECTION 1: CART ITEMS ── */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
        {cart.map((item, idx) => (
          <div key={idx} style={{ display: 'flex', gap: 12, padding: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, alignItems: 'center' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '20px 18px', marginBottom: 24 }}>
              ...
              {cryptoEnabled && (
                <button onClick={() => setPayTab('CRYPTO')}
                  style={{ flex: 1, padding: '8px 12px', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer', border: '1px solid', background: 'transparent',
                    borderColor: payTab === 'CRYPTO' ? 'rgba(0,245,255,0.4)' : 'rgba(255,255,255,0.08)',
                    color: payTab === 'CRYPTO' ? '#00F5FF' : 'rgba(255,255,255,0.4)' }}>
                  ₮ Cripto (USDT)
                </button>
              )}
              {manualEnabled && (
