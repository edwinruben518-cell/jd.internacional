    ...
    <div className="px-4 sm:px-6 pt-6 pb-12 max-w-7xl mx-auto">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 28 }} className="sm:grid-cols-2">
          ...
          <div style={{ aspectRatio: '1/1', borderRadius: 16, overflow: 'hidden', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', position: 'relative' }}>
            {images.length > 0 ? (
              ...
              <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg className="w-16 h-16 opacity-20" viewBox="0 0 24 24" fill="none" stroke="#00F5FF" strokeWidth="1"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007z" /></svg>
              </div>
            )}
            {outOfStock && (
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
              {images.map((src, i) => (
                <button key={i} onClick={() => setActiveImg(i)}
                  style={{ width: 56, height: 56, borderRadius: 8, overflow: 'hidden', border: `2px solid ${activeImg === i ? '#00F5FF' : 'rgba(255,255,255,0.1)'}`, background: 'none', cursor: 'pointer', padding: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#00F5FF', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{item.category}</span>
